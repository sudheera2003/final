...
export default function ChatbotPage() {
  ...
  const loadChatHistory = async (sessionId: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/chat/history/${sessionId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
    ...
    setMessages([
      {
        role: "assistant",
        content:
          "Hello! I am **RestoAI**, your Lady Hill Hotel management assistant. I'm connected directly to your live inventory and sales data. How can I help you today?",
      },
    ]);
  };
  ...
  const handleDeleteChat = async (sessionId: string, e: React.MouseEvent) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/chat/${sessionId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      <div className="w-64 border-r bg-muted/10 hidden md:flex flex-col shrink-0">
        ...
        <ScrollArea className="flex-1 px-2 pb-4">
          <div className="space-y-1">
            <p className="px-3 text-xs font-semibold text-muted-foreground mb-2 mt-4">
            ...
            {sessions.map((session) => (
              ...
              >
                ...
                <button
                  ...
                  className="text-muted-foreground hover:text-destructive transition-opacity cursor-pointer"
                >
        ...
        <div className="border-b bg-background/95 backdrop-blur px-6 py-4 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Live Context-Aware Management AI
            </p>
          </div>
        </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center mt-3">
              <span className="text-[10px] text-muted-foreground">
                RestoAI can make mistakes. Verify critical stock decisions on
                the dashboard.
              </span>
            </div>
          </div>
